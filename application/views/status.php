...
<div style="vertical-align:middle; text-align:center">
    <form class="form-inline" method="get" action="/index.php/judge/status/<?=$class_id?>/<?=$contest_id?>/<?=$this->uri->segment(5)?>">
			...
			?>
                	...
                	<td style="vertical-align:middle; text-align:center; color:#e67e22; "> 
                        <?php if($entry->result==1){
                               $btncl = 'Judging '.round($entry->progress_rate*100).'% <img width="30" src="/static/img/loading_spinner.gif">';
				                ...
				                $res ='Compile Error';
				                if($this->session->userdata('user_id') == $entry->userID){
                                    $btncl = '<a href="/index.php/judge/error/'.$class_id.'/'.$entry->C_P_ID.'/'.$entry->ID.'" class="btn btn-danger btn-sm">'.$res.'</a>';    
                                }
                                else{
                                    $btncl = '<a class="btn btn-danger btn-sm">'.$res.'</a>';
                                }
                ...
                <?php  }else if($entry->pass_rate*100 == 100) {?>
                	...
                	<td style="vertical-align:middle; text-align:center"> 
                        <?php 
                            if($this->session->userdata('user_id') == $entry->userID){ ?>
                                <a href="/index.php/judge/showsource/<?=$class_id?>/<?=$entry->C_P_ID?>/<?=$entry->ID?>">
                        <?php } ?>
                        ...
                        <?php if($entry->language==1){
                            ...
                            <?=$lang?>
                        <?php if($this->session->userdata('user_id') == $entry->userID){ ?>
                            </a>
                             / 
                            <a href="/index.php/judge/submitpage/<?=$class_id?>/<?=$contest_id?>/<?=$entry->C_P_ID?>?sid=<?=$entry->ID?>">edit</a>
                        <?php } ?>
                    </td>
    ...
    function fresh_result(i){
        $.ajax({
            ...
            success:function(data){
                if(data.result == 1){
                    rows[i].cells[2].innerHTML = "Judging ("+parseInt(data.progress_rate*100)+"%) <img width='30' src='/static/img/loading_spinner.gif'>";
                    ...
                    if(data.result == 2)
                        ...
                        if("<?=$this->session->userdata('user_id')?>" == data.userID)
                            rows[i].cells[2].innerHTML = "<a href='/index.php/judge/error/<?=$class_id?>/"+data.C_P_ID+"/"+data.ID+"' class='btn btn-danger btn-sm'>Compile Error</a>";
                        else
