...
<div style="vertical-align:middle; text-align:center">
    <form class="form-inline" method="get" action="/index.php/manager/teststatus/<?=$id?>/<?=$problem_id?>/<?=$mode?>">
			...
			?>
                	...
                	<td style="vertical-align:middle; text-align:center; color:#e67e22;"> 
                        <?php if($entry->result==1){
                               $btncl = 'Judging '.round($entry->progress_rate*100).'% <img width="30" src="/static/img/loading_spinner.gif">';
				                ...
				                $res ='Compile Error';
                                $btncl = '<a href=" /index.php/manager/testerror/'.$id.'/'.$entry->problemID.'/'.$entry->ID.'/'.$mode.' " class="btn btn-danger btn-sm">'.$res.'</a>';
                ...
                <?php  }else if($entry->pass_rate*100 == 100) {?>
                	...
                	<td style="vertical-align:middle; text-align:center"> 
                        ...
                        <a href="/index.php/manager/testshowsource/<?=$id?>/<?=$entry->problemID?>/<?=$entry->ID?>/<?=$mode?>">
                        ...
                        <a href="/index.php/manager/testsubmitpage/<?=$id?>/<?=$entry->problemID?>/<?=$mode?>?sid=<?=$entry->ID?>">edit</a>
                    </td>
                ...
                if(data.result == 1){
                    ...
                    if(data.result == 2)
                        ...
                        if("<?=$this->session->userdata('user_id')?>" == data.userID)
                            rows[i].cells[2].innerHTML = "<a href='/index.php/manager/testerror/<?=$id?>/"+data.problemID+"/"+data.ID+"/<?=$mode?>' class='btn btn-danger btn-sm'>Compile Error</a>";
                        else
